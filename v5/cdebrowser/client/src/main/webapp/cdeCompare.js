...
angular.module("cdeCompare").controller("cdeCompareController",  ["$scope", "$http", "$window", "$location", function ( $scope, $http, $window, $location) {
    window.scope = $scope;
   ...
   // $scope.compareDataDoneLoading = false;
    ...
    $scope.multipleCdeDetails = function () {
        ...
        $scope.compareDataDoneLoading = false;
        $scope.getCdeDetailRestCall(window.location.protocol + "//" + window.location.hostname + ":" + window.location.port + "/cdebrowserServer/rest/multiCDEData?deIdseq=" + idList);
    };
