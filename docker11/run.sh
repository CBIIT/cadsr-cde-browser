...
mvn -f v5/cdebrowser/server/pom.xml -DCADSR_DS_USER=${CADSR_DS_USER} -DCADSR_DS_PSWD=${CADSR_DS_PSWD} -Dtag=${tag} -Dtier=${tier} -Djavax.xml.parsers.DocumentBuilderFactory=com.sun.org.apache.xerces.internal.jaxp.DocumentBuilderFactoryImpl -P AHP -s v5/cdebrowser/settings.xml clean package
